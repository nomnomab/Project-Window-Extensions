...
namespace Nomnom.ProjectWindowExtensions.Editor.Folder {
	internal class TypeWindow : EditorWindow {
		...
		public static void Open(Vector2 mousePos, Vector2 size, Action<string> onClicked) {
			TypeWindow window = CreateInstance<TypeWindow>();
			mousePos.y -= size.y;
			Rect rect = new Rect(mousePos, size);
			window._onClicked = onClicked;
			
			_typeList = PreferenceWindow.FolderImportAssemblies
		...
		private void OnGUI() {
			...
			if (_results == null) {
				EditorGUILayout.LabelField("Input a type name to list types", EditorStyles.miniBoldLabel);
			} else {
